...
class Forage:
    ...
    def update(self):
        ...
        global dead_scout_ID
        global scout_hitlist
        global dead_exploit_ID
        global exploit_hitlist
        
        #print(len(self.Exploiters)+len(self.Explorers))
        #explorer deaths
        
        while len(scout_hitlist) !=0:
            #print(len(scout_hitlist))
            index_count = 0
            dead_b_explorer = []
            for i in self.Explorers:
                dictionary = i.__dict__
                
                important_ID = dictionary['BeeId']
                
                for ii in scout_hitlist:
                    if ii == important_ID:
                        
                        #kill that little bee
                        del self.Explorers[index_count]
                        dead_b_explorer.append(index_count)
                        scout_hitlist.remove(ii)
                        dead_scout_ID.append(important_ID) #add to list of free ids
                        #print("a bee was killed")
                index_count = index_count+1
        #print(len(self.Exploiters)+len(self.Explorers))
        
        ##this is where baby bees come from
        bee_birthrate = 0.015
        scouts_born = len(self.Explorers)*bee_birthrate
        #scout bees
        scout_babies = 0
        while len(dead_scout_ID) != 0 and scout_babies <= scouts_born:
            #print("this should add a bee") #debugging
            w = dead_scout_ID[0]
            self.Explorers.append(    # to make each explorer, create an object in class "honeybee" with scout parameters
                HoneyBee(w, 0, 0,          # mode exploring, bclass scout
                         self.Lx/2, self.Ly/2, # starting location is hive position
                         v = 1.5, tsigma = 5., # scout velocity and error
                         persistence = 20) # scout persistence
                    
            )
            self.Explorers[-1].Hive = self  # each new bee has an attribute Hive, which is set equal to the name of the Forage instance
            dead_scout_ID.remove(dead_scout_ID[0])
            scout_babies = scout_babies+1
            #print("a bee was born")
            
        #print(len(self.Exploiters) + len(self.Explorers)) #debugging
        #print("bees2")
        #print(len(self.Exploiters)+len(self.Explorers))
        ...
        for bee in self.Explorers:
            ...
            if ( bee.mode == 0 or bee.mode == -3 ):
                if self.Area3[int(bee.x),int(bee.y)].sum() > 0 :
                    ...
                    if lottery_num == 1 or lottery_num == 2 or lottery_num == 3:
                        #print("bee dead")
                        bee_dict = bee.__dict__
                        unlucky_bee_ID = bee_dict['BeeId']
                        scout_hitlist.append(unlucky_bee_ID)
                        #print(scout_hitlist)
                    ...
                    #20% predation
                    #density 2 predation - 20%
                    
                    lottery_num = random.randint(0,9)
                    if lottery_num == 1 or lottery_num == 2:
                        #print("bee dead")
                        bee_dict = bee.__dict__
                        unlucky_bee_ID = bee_dict['BeeId']
                        scout_hitlist.append(unlucky_bee_ID)
        ...
        #exploiter deaths
        while len(exploit_hitlist) !=0:
            #print(len(exploit_hitlist))
            index_count = 0
            for i in self.Exploiters:
                dictionary = i.__dict__
                
                important_ID = dictionary['BeeId']
                
                for ii in exploit_hitlist:
                    if ii == important_ID:
                        #print(dictionary)
                        
                        ##kill that little bee
                        #print(self.Exploiters[index_count])
                        del self.Exploiters[index_count] #RIP
                        #print(len(self.Exploiters))
                        exploit_hitlist.remove(ii)
                        dead_exploit_ID.append(important_ID) #add to list of free ids
                        #print("a bee is deadIT WORKED")
                        #print(len(self.Explorers)+len(self.Exploiters))
                index_count = index_count+1
            ...
            if ( bee.mode == 0 or bee.mode == -3 ) :
                if self.Area3[int(bee.x),int(bee.y)].sum() > 0 :
                    ...
                    if lottery_num == 1 or lottery_num == 2 or lottery_num == 3:
                        ...
                        bee_dict = bee.__dict__
                        unlucky_bee_ID = bee_dict['BeeId']
                        exploit_hitlist.append(unlucky_bee_ID)
        ...
        self.t += 1
                ...
                #Rebrith of bees - added - lindsay
        #this is where bees come from part 2
        
        exploits_born = len(self.Exploiters)*bee_birthrate
        
        
        #exploiter bees
        exploit_babies = 0
        while len(dead_exploit_ID) != 0 and exploit_babies <= exploits_born:
            #print("this should add a bee") #debugging
            k = dead_exploit_ID[0]
            self.Exploiters.append(
                HoneyBee(k, 1, 1,            # mode waiting, bclass recruit
                         self.Lx/2, self.Ly/2+2, # starting location is hive position, why are these bees starting slightly away from hive?
                         v = 1.0, tsigma = 2.,   # recruit velocity and error
                         persistence = 20 ) # recruit persistence SET TO 20 CHNAGE THIS AS NEEDED
            )
            self.Exploiters[-1].Hive = self
            dead_exploit_ID.remove(dead_exploit_ID[0])
            exploit_babies = exploit_babies+1
            #print("A BABE IS BORN")
